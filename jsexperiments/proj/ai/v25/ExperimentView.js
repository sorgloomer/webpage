...
ExperimentView.prototype.drawPlans = function(ctx, _opts) {
	...
	var lastNotJustPath = this.pathDrawer.scorer.tempWorld.notJustPath;
	this.pathDrawer.scorer.notJustPath = false;
	if (_opts.showPlans === "all") {
		for (i = arr.length - 1; i > 0; i--) {
			this.pathDrawer.drawPath(ctx, arr[i]);
		}
	}
	if (_opts.showPlans === "all" || _opts.showPlans === "best") {
		this.pathDrawer.drawPath(ctx, arr[0], 0.5);
	}
	this.pathDrawer.scorer.tempWorld.notJustPath = lastNotJustPath;
};
