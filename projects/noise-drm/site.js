...
class GlContext {
  ...
  makeShader(fShaderStr, vShaderStr, attrs, unis) {
    var fragmentShader = this.compileShader(gl.FRAGMENT_SHADER, fShaderStr);
    var vertexShader = this.compileShader(gl.VERTEX_SHADER, vShaderStr);
    ...
    if (!this.gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) {
      ...
      this.error("Could not initialise shaders: " + infoLog);
    } else {
...
function extract(shaderCode, type) {
  ...
  var regexp = new RegExp(`\\A\\w*${escapeRegex(type)}\\w+${identifier}\\w+${identifier};\\w*\\Z`, 'g');
  var matches = shaderCode.matchAll(regexp);
...
function Shader(program, idVertex, idFragment) {
